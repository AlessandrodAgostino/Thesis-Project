...
% \hl{Comment to adjust}
% Although this strategy produces realistic-looking samples, the generation of the starting tumoral pattern is ruled by the random sampling of sparse seed points, the distribution of which should be finely tuned for the production of an accurate representation. Furthermore, this procedure is based completely on the phenomenological replication of the image, without any try to capture the spatial organization of an actual tissue ....
% \hl{Comment to adjust}

The second work I want to mention is base on a DL-base technique, which approaches synthetic image generation using a specific cGAN architecture inspired to the ``U-net'' \cite{Senaras2018} model described back in Figure \ref{fig:unet} in section \ref{ssec:soa_seg}.
